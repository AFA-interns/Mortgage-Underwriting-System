        ...
        if salary_slips and bank_statement:
            ...
            for slip in salary_slips:
                ...
                if variance_pct <= 5.0:
                    status = "MATCH"
                    is_reconciled = True
                    notes = f"Bank salary credit (₹{credited_amount:,.2f}) matches payslip net salary (₹{payslip_net:,.2f})."
                elif variance_pct <= 15.0:
                    status = "WITHIN_TOLERANCE"
                    is_reconciled = True
                    notes = f"Slight variance of {variance_pct}% (₹{variance_amount:,.2f}) due to variable components or tax adjustment."
                else:
                    status = "DISCREPANCY"
                    is_reconciled = False
                    notes = f"Large variance: Bank credit ₹{credited_amount:,.2f} vs Payslip Net ₹{payslip_net:,.2f} ({variance_pct}% difference)."
                    contradictions.append(
                        f"Income discrepancy in {slip_month}: Bank credit (₹{credited_amount:,.2f}) differs significantly from Payslip Net (₹{payslip_net:,.2f})."
                    )
