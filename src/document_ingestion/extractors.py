...
from src.document_ingestion.preprocessor import PreprocessedDocument
...
class DocumentExtractors:
    ...
    def extract_salary_slip(cls, doc: PreprocessedDocument) -> SalarySlipData:
        ...
        def _parse_amount(pattern: str, src_text: str) -> Optional[float]:
            ...
            if match:
                raw_val = match.group(1).replace(",", "").replace("₹", "").replace("Rs.", "").strip()
                try:
        ...
        gross_salary = _parse_amount(r"(?:Gross\s*(?:Salary|Earnings|Pay|Amount)?)\s*[:\-]?\s*(?:INR|Rs\.?|₹)?\s*([\d,]+\.?\d*)", text) or 0.0
        net_salary = _parse_amount(r"(?:Net\s*(?:Salary|Pay|Amount|Take\s*Home)?)\s*[:\-]?\s*(?:INR|Rs\.?|₹)?\s*([\d,]+\.?\d*)", text) or 0.0
        total_deductions = _parse_amount(r"(?:Total\s*Deductions?)\s*[:\-]?\s*(?:INR|Rs\.?|₹)?\s*([\d,]+\.?\d*)", text) or 0.0

        if gross_salary > 0:
            provenance["gross_salary"] = FieldProvenance(
                source_document=doc.filename,
                page_number=1,
                raw_snippet=f"Gross Salary: ₹{gross_salary:,.2f}",
                confidence=0.95
            )
        if net_salary > 0:
            provenance["net_salary"] = FieldProvenance(
                source_document=doc.filename,
                page_number=1,
                raw_snippet=f"Net Salary: ₹{net_salary:,.2f}",
                confidence=0.96
            )
        if total_deductions > 0:
            provenance["total_deductions"] = FieldProvenance(
                source_document=doc.filename,
                page_number=1,
                raw_snippet=f"Total Deductions: ₹{total_deductions:,.2f}",
                confidence=0.93
            )
        ...
        earning_patterns = [
            ("Basic Pay", r"(?:Basic(?:\s*Pay|\s*Salary)?)\s*[:\-]?\s*(?:INR|Rs\.?|₹)?\s*([\d,]+\.?\d*)"),
            ("HRA", r"(?:House\s*Rent\s*Allowance|HRA)\s*[:\-]?\s*(?:INR|Rs\.?|₹)?\s*([\d,]+\.?\d*)"),
            ("Special Allowance", r"(?:Special\s*Allowance|Spl\s*Allow)\s*[:\-]?\s*(?:INR|Rs\.?|₹)?\s*([\d,]+\.?\d*)"),
            ("Conveyance Allowance", r"(?:Conveyance|Transport\s*Allowance)\s*[:\-]?\s*(?:INR|Rs\.?|₹)?\s*([\d,]+\.?\d*)"),
            ("Medical Allowance", r"(?:Medical\s*Allowance)\s*[:\-]?\s*(?:INR|Rs\.?|₹)?\s*([\d,]+\.?\d*)"),
        ]

        deduction_patterns = [
            ("Provident Fund (PF)", r"(?:Provident\s*Fund|EPF|PF)\s*[:\-]?\s*(?:INR|Rs\.?|₹)?\s*([\d,]+\.?\d*)"),
            ("Professional Tax (PT)", r"(?:Professional\s*Tax|PT)\s*[:\-]?\s*(?:INR|Rs\.?|₹)?\s*([\d,]+\.?\d*)"),
            ("Income Tax / TDS", r"(?:Income\s*Tax|TDS|Tax\s*Deducted)\s*[:\-]?\s*(?:INR|Rs\.?|₹)?\s*([\d,]+\.?\d*)"),
            ("Loan Deduction", r"(?:Loan\s*(?:EMI|Deduction|Recovery))\s*[:\-]?\s*(?:INR|Rs\.?|₹)?\s*([\d,]+\.?\d*)"),
        ]
    ...
    def extract_form_16(cls, doc: PreprocessedDocument) -> Form16Data:
        ...
        def _get_val(pat: str) -> float:
            ...
            if m:
                raw = m.group(1).replace(",", "").replace("Rs.", "").replace("₹", "").strip()
                try:
                    return float(raw)
                except ValueError:
                    return 0.0
        ...
        gross_salary_sec17_1 = _get_val(r"(?:Gross\s*Salary[^\n:]*|Salary\s*as\s*per\s*provisions\s*contained\s*in\s*section\s*17\(1\))\s*[:\-]?\s*(?:INR|Rs\.?|₹)?\s*([\d,]+\.?\d*)")
        total_deductions_chapter_via = _get_val(r"(?:Total\s*deductions[^\n:]*|Aggregate\s*of\s*deductible\s*amount\s*under\s*Chapter\s*VI-A)\s*[:\-]?\s*(?:INR|Rs\.?|₹)?\s*([\d,]+\.?\d*)")
        taxable_income = _get_val(r"(?:Total\s*Taxable\s*Income|Taxable\s*Income|Income\s*chargeable\s*under\s*the\s*head\s*'Salaries')\s*[:\-]?\s*(?:INR|Rs\.?|₹)?\s*([\d,]+\.?\d*)")
        tax_deducted_total = _get_val(r"(?:Total\s*(?:TDS|Tax\s*Deducted)|Tax\s*payable|Total\s*amount\s*of\s*tax\s*deducted\s*at\s*source)\s*[:\-]?\s*(?:INR|Rs\.?|₹)?\s*([\d,]+\.?\d*)")

        if gross_salary_sec17_1 > 0:
            provenance["gross_salary_sec17_1"] = FieldProvenance(
                source_document=doc.filename,
                page_number=1,
                raw_snippet=f"Form 16 Gross: ₹{gross_salary_sec17_1:,.2f}",
                confidence=0.95
            )
    ...
    def extract_itr(cls, doc: PreprocessedDocument) -> ITRVData:
        ...
        gross_inc = _val(r"(?:Gross\s*Total\s*Income|Total\s*Income)\s*[:\-]?\s*(?:INR|Rs\.?|₹)?\s*([\d,]+\.?\d*)")
        taxable_inc = _val(r"(?:Total\s*Taxable\s*Income|Net\s*Taxable\s*Income)\s*[:\-]?\s*(?:INR|Rs\.?|₹)?\s*([\d,]+\.?\d*)")
        deductions = _val(r"(?:Total\s*Deductions|Deductions\s*under\s*Chapter\s*VI-A)\s*[:\-]?\s*(?:INR|Rs\.?|₹)?\s*([\d,]+\.?\d*)")
        tax_paid = _val(r"(?:Total\s*Tax\s*(?:Paid|Payable)|Taxes\s*Paid)\s*[:\-]?\s*(?:INR|Rs\.?|₹)?\s*([\d,]+\.?\d*)")
