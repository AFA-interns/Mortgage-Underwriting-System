...
class IndianDocumentValidators:
    ...
    def validate_salary_arithmetic(
        ...
        if variance > 2.0:  # Allow small rounding threshold of ₹2
            return ValidationResult(
                False,
                "salary_math",
                f"Payslip arithmetic mismatch: Gross (₹{gross_salary:,.2f}) - Deductions (₹{total_deductions:,.2f}) = ₹{expected_net:,.2f}, but Net Pay is ₹{net_salary:,.2f} (Variance: ₹{variance:,.2f})",
                "ERROR"
            )
        ...
        if earnings_sum and abs(earnings_sum - gross_salary) > 5.0:
            return ValidationResult(
                True,
                "salary_math",
                f"Sum of itemized earnings (₹{earnings_sum:,.2f}) differs slightly from Gross Salary (₹{gross_salary:,.2f}).",
                "WARNING"
            )
