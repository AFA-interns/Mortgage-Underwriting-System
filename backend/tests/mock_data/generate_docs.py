...
def generate_all_mock_scenarios(base_dir: str = "mock_documents") -> Dict[str, List[str]]:
    ...
    # 2. Aadhaar Card
    aadhaar_p = os.path.join(s1_dir, "aadhaar_card_aarav_sharma.pdf")
    _create_pdf_document(aadhaar_p, [{
        ...
        "sections": [
            {
                ...
                "items": [
                    ...
                    ("Date of Birth", "14/08/1990"),
                    ("Gender", "MALE"),
                    ("Address", "Flat 402, Palm Meadows, Whitefield, Bengaluru, Karnataka, PIN: 560066"),
                    ("Pincode", "560066"),
    ...
    # 3. Salary Slips (April, May, June 2026)
    for month in ["April 2026", "May 2026", "June 2026"]:
        month_fn = month.lower().replace(" ", "_")
        sal_p = os.path.join(s1_dir, f"salary_slip_{month_fn}.pdf")
        _create_pdf_document(sal_p, [{
            ...
            "sections": [
                ...
                },
                {
                    ...
                    "table": {
                        "headers": ["Earnings Component", "Amount (INR)", "Deductions Component", "Amount (INR)"],
                        "rows": [
                            ["Basic Pay", "60,000.00", "Provident Fund (PF)", "7,200.00"],
                            ["House Rent Allowance (HRA)", "30,000.00", "Professional Tax (PT)", "200.00"],
                            ["Special Allowance", "45,000.00", "Income Tax (TDS)", "18,000.00"],
                            ["Conveyance Allowance", "15,000.00", "Voluntary Deductions", "0.00"]
                        ]
                    }
                }
            ],
    ...
    # 4. Form 16
    f16_p = os.path.join(s1_dir, "form_16_ay_2026_27.pdf")
    _create_pdf_document(f16_p, [{
    ...
    # 5. Bank Statement (6 Months - HDFC Bank)
    stmt_p = os.path.join(s1_dir, "bank_statement_6m_hdfc.pdf")
    _create_pdf_document(stmt_p, [{
        ...
        "sections": [
            {
                ...
                "items": [
                    ...
                    ("Branch", "Whitefield Bangalore Branch"),
            ...
            },
            {
                ...
                "table": {
                    "headers": ["Date", "Description / Narration", "Chq/Ref No", "Debit (INR)", "Credit (INR)", "Balance (INR)"],
                    "rows": [
                        ["01/04/2026", "SALARY CREDIT TCS PAYROLL", "CMS00984", "0.00", "1,24,600.00", "2,10,000.00"],
                        ["05/04/2026", "ACH DEBIT HDFC AUTO LOAN EMI", "ACH48392", "15,000.00", "0.00", "1,95,000.00"],
                        ["01/05/2026", "SALARY CREDIT TCS PAYROLL", "CMS01124", "0.00", "1,24,600.00", "2,28,000.00"],
                        ["05/05/2026", "ACH DEBIT HDFC AUTO LOAN EMI", "ACH48392", "15,000.00", "0.00", "2,13,000.00"],
                        ["01/06/2026", "SALARY CREDIT TCS PAYROLL", "CMS01348", "0.00", "1,24,600.00", "2,55,000.00"],
                        ["05/06/2026", "ACH DEBIT HDFC AUTO LOAN EMI", "ACH48392", "15,000.00", "0.00", "2,40,000.00"]
                    ]
                }
            }
        ],
    ...
    deed_p = os.path.join(s1_dir, "property_sale_deed_bengaluru.pdf")
    _create_pdf_document(deed_p, [{
        ...
        "sections": [
            {
                ...
                "items": [
                    ...
                    ("Registration Number", "BNG-VRT-4458/2026"),
                    ("Vendor / Developer", "Prestige Estates Projects Ltd"),
                    ("Purchaser / Buyer", "Aarav Sharma"),
                    ("Property Type", "Residential Apartment Unit")
                ]
            },
            {
                "heading": "Schedule Property Specifications",
                "items": [
                    ("Property Address", "Unit 604, Tower 3, Prestige Lakeside Habitat, Varthur Main Road, Bengaluru, Karnataka, PIN: 560087"),
                    ("Super Built-up Area", "1,450.00 Sq. Ft."),
                    ("Carpet Area", "1,120.00 Sq. Ft."),
                    ("Consideration Amount", "INR 1,25,00,000.00"),
                    ("Stamp Duty Paid", "INR 6,25,000.00")
                ]
            }
        ],
        "footer": "Office of the Sub-Registrar, Varthur, Bengaluru | Registered & Sealed"
    }])
    ...
    # =========================================================================
    # SCENARIO 2: Name Discrepancy Case (Priya S Patel vs Priya Suresh Patel)
    # =========================================================================
    s2_dir = os.path.join(base_dir, "scenario_2_name_discrepancy")
    os.makedirs(s2_dir, exist_ok=True)
    s2_files = []

    # PAN: Priya Suresh Patel
    p_pan = os.path.join(s2_dir, "pan_priya_patel.pdf")
    _create_pdf_document(p_pan, [{
        "title": "INCOME TAX DEPARTMENT - GOVT. OF INDIA",
        "subtitle": "Permanent Account Number Card (PAN)",
        "sections": [{
            "heading": "Taxpayer Information",
    ...
    # Aadhaar: Priya S. Patel (Abbreviated middle name)
    p_aadh = os.path.join(s2_dir, "aadhaar_priya_s_patel.pdf")
    _create_pdf_document(p_aadh, [{
        ...
        "sections": [{
            ...
            "items": [
                ...
                ("Date of Birth", "22/11/1993"),
                ("Gender", "FEMALE"),
                ("Address", "A-302, Green Glen Layout, Bellandur, Bengaluru, Karnataka, PIN: 560103")
            ]
        }]
    }])
    s2_files.append(p_aadh)

    # Salary Slip: Priya Patel (Missing middle name)
    p_sal = os.path.join(s2_dir, "salary_slip_priya.pdf")
    _create_pdf_document(p_sal, [{
    ...
    # =========================================================================
    # SCENARIO 3: Income / Salary vs Bank Credit Discrepancy (Vikram Malhotra)
    # =========================================================================
    s3_dir = os.path.join(base_dir, "scenario_3_salary_discrepancy")
    os.makedirs(s3_dir, exist_ok=True)
    s3_files = []

    # PAN
    v_pan = os.path.join(s3_dir, "pan_vikram_malhotra.pdf")
    _create_pdf_document(v_pan, [{
        "title": "INCOME TAX DEPARTMENT - GOVT. OF INDIA",
    ...
    # Salary Slip: Claims Net Pay of Γé╣1,60,000
    v_sal = os.path.join(s3_dir, "salary_slip_vikram.pdf")
    _create_pdf_document(v_sal, [{
    ...
    # Bank Statement: Only Γé╣85,000 salary credit deposited (Γé╣75,000 mismatch!)
    v_bank = os.path.join(s3_dir, "bank_statement_vikram.pdf")
    _create_pdf_document(v_bank, [{
        ...
        "sections": [
            ...
            },
            {
                ...
                "table": {
                    "headers": ["Date", "Narration", "Debit (INR)", "Credit (INR)"],
                    "rows": [
                        ["01/06/2026", "SALARY CREDIT APEX RETAIL", "0.00", "85,000.00"],
                        ["01/05/2026", "SALARY CREDIT APEX RETAIL", "0.00", "85,000.00"],
                        ["01/04/2026", "SALARY CREDIT APEX RETAIL", "0.00", "85,000.00"]
                    ]
                }
            }
        ]
    ...
    # =========================================================================
    # SCENARIO 4: Missing Mandatory Documents (Rahul Verma)
    # =========================================================================
    s4_dir = os.path.join(base_dir, "scenario_4_missing_docs")
    os.makedirs(s4_dir, exist_ok=True)
    s4_files = []

    # Only PAN card and 1 Salary slip provided
    r_pan = os.path.join(s4_dir, "pan_rahul_verma.pdf")
    _create_pdf_document(r_pan, [{
        "title": "INCOME TAX DEPARTMENT - GOVT. OF INDIA",
        "subtitle": "Permanent Account Number Card",
        "sections": [{
            "heading": "Taxpayer Information",
    ...
    r_sal = os.path.join(s4_dir, "salary_slip_rahul.pdf")
    _create_pdf_document(r_sal, [{
        "title": "WIPRO LIMITED",
        "subtitle": "Payslip for June 2026",
        "sections": [{
            "heading": "Employee Earnings",
